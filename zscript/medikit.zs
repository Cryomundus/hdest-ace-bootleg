...
class HDMedikitter:HDWoundFixer{
	...
	override void DrawHUDStuff(HDStatusBar sb,HDWeapon hdw,HDPlayerPawn hpl){
		...
		if(usedon>=0){
			...
			(
				...
				target?"\ce"..HDMath.GetName(target):
				"\ca*** UNKNOWN ***"
			);
		...
		if(bt&BT_ATTACK){
			...
			if(blockinv){
				if(getcvar("hd_autostrip"))setweaponstate("reload");
					...
					if(getcvar("hd_helptext"))A_WeaponMessage("Take off your "..blockinv.gettag().." first!\n\n(toggle it in your inventory or hit reload)",100);
					setweaponstate("nope");
				}
				return;
			}
	...
	fireother:
		...
		TNT1 A 10{
			...
			let blockinv=HDWoundFixer.CheckCovered(c,CHECKCOV_CHECKBODY);
			if(
				!c.player.bot
				&&blockinv
			){
				if(getcvar("hd_helptext"))A_WeaponMessage("Get them to take off their "..blockinv.gettag().." first!\n\n(\cdhd_strip\c- in the console)",100);
				return resolvestate("nope");
			}
			if(
	...
	action void A_ScanResults(actor scanactor,int scanaccuracy){
		...
		if(!slf){
			int scanactorhealthpercent=
				(
					hdmobbase(scanactor)?
					min(
						scanactor.health,
						scanactor.spawnhealth()-hdmobbase(scanactor).bodydamage
					)
					:scanactor.health
				)
				*100/scanactor.spawnhealth();
			A_WeaponMessage(string.format("Medikit Auto-Diagnostic for
			\cy%s

			\ccOverall Health: \cg%u%%

			\cu(%% based on IGIC-IV diagnostic guidelines)",

			scanactorname,scanactorhealthpercent+random(-thrownoff,thrownoff)),210);

			A_Log(string.format("Medikit Auto-Diagnostic for \cy%s
\ccOverall Health: \cg%u%%",scanactorname,scanactorhealthpercent),true);
		...
		if(ww||uw)wg+=2;
		if(HDWoundFixer.CheckCovered(self,CHECKCOV_CHECKBODY))wg+=5;
		...
		//the International Gaming Industry Consortium is the world's leading standards
		//regulator for medical supplies in all freedom-respecting G20 nations.
...
class SelfBandage:HDWoundFixer{
	...
	void bandagewound(int amt,actor targ){
		let slf=HDPlayerPawn(targ);
		if(slf){
			if(!random(0,2)&&(slf.bspecialfiredamage||slf.bshadow))amt-=random(0,amt+1);
			int wound=max(slf.woundcount,0);
			amt=min(amt,wound);
			if(wound){
				amt=min(amt,wound);
	...
	fire:
		TNT1 A 0{
			...
			let blockinv=HDWoundFixer.CheckCovered(self,CHECKCOV_ONLYFULL);
			if(blockinv){
				if(getcvar("hd_helptext"))A_WeaponMessage("Take off your "..blockinv.gettag().." first!",70);
				nope=true;
			}
