	...
	Spawn:
		...
		TNT1 A 1
		{
			...
			if (tg.CountInv("HDZerk") > HDZerk.HDZERK_COOLOFF + 666)
			{
				if (HDPlayerPawn(tg))
				{
					tg.A_StartSound(HDPlayerPawn(tg).xdeathsound, CHAN_VOICE);
				}
				else
				{
			...
			else
			{
				if (HDPlayerPawn(tg))
				{
					tg.A_StartSound(HDPlayerPawn(tg).PainSound, CHAN_VOICE);
				}
				else tg.A_StartSound("*pain", CHAN_VOICE);
			}
			if (tg.CountInv("HDStim"))
			{
				tg.aggravateddamage += int(ceil(tg.CountInv("HDStim") * 0.05 * random(1, 3)));
			}
...
class HDBlues:HDDrug{
	...
	override void pretravelled(){
		...
		int toheal = min(
			amount, 
			hdp.woundcount + hdp.unstablewoundcount, hdp.bloodloss / 12
		);
		if (toheal > 0){
			int wc = min(hdp.woundcount, toheal);
			hdp.woundcount -= wc;
			toheal -= wc;
			if (toheal > 0){
				wc = min(hdp.unstablewoundcount, toheal);
				hdp.unstablewoundcount -= wc;
				toheal -= wc;
			}
			if (toheal > 0){
				wc = min(hdp.bloodloss, toheal * 12);
				hdp.bloodloss -= wc;
				toheal -= wc / 12;
			}
		}
	...
	override void OnHeartbeat(HDPlayerPawn hdp){
		...
		if (
			...
			if (hdp.woundcount > 0)hdp.woundcount--;else hdp.unstablewoundcount--;
			amount--;
		}
