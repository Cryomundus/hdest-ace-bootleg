...
class HDWeapon:Weapon{
	...
	action int GetMouseX(bool hijack=false){
		if(hijack)reactiontime=max(reactiontime,1);
	...
	virtual void actualpickup(actor other,bool silent=false){
		...
		if(oldwep){
			...
			if(
				...
				if(!silent)other.A_Log(string.format("Playing low encumbrance mode. Drop your current "..gettag().." first."),true);
				return;
			}
			...
			if(
				...
				if(asw&&!silent&&!!self){
					...
					if(
						...
						hdp.A_Log("\caThis is your "..gettag().." number "..(wepcount+1)..".",true);
					}
				}
				return;
			}
